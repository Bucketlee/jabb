    ...
    <div className="flex items-end gap-0.5 h-20 w-full">
      {filled.map((item) => {
        ...
        return (
          ...
          >
            <div
              className="w-full bg-zinc-900 dark:bg-zinc-100 rounded-sm opacity-80 group-hover:opacity-100 transition-all"
              style={{ height: `${Math.max(heightPct, 2)}%` }}
            />
            <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 text-xs px-1.5 py-0.5 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-10">
